    ...
    for (int i = 0; i < grid_h; ++i) {
        for (int j = 0; j < grid_w; ++j) {
            ...
            if (box_confidence >= thres_i8) {
                ...
                float combined = deqnt_affine_to_f32(maxClassProbs, zp, scale) * deqnt_affine_to_f32(box_confidence, zp, scale);
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            if (box_confidence >= threshold)
            {
                ...
                float combined = maxClassProbs * box_confidence;
                if (combined > threshold)
                {
                    objProbs.push_back(combined);
                    classId.push_back(maxClassId);
