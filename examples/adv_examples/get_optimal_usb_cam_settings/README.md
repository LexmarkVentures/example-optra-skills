...
### Natively (ARM64):
```sh
docker build --platform linux/arm64 -t <registry>/get-cam-settings:<skill_tag> --push .
```

### Non-natively (x64):
```sh
docker buildx build --platform linux/arm64 -t <registry>/get-cam-settings:<skill_tag> --push .
```
